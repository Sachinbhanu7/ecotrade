...
import { ShieldAlert, LayoutDashboard, LogOut, CheckCircle, XCircle, Crown, Users, TrendingUp, X, IdCard } from 'lucide-react';
...
const Dashboard = ({ setAuth }) => {
  ...
  const [stats, setStats] = useState({ totalUsers: 0, totalItems: 0, pendingApprovals: 0, totalBids: 0, pendingKyc: 0 });
  ...
  const kycPendingUsers = users.filter(u => u.kycStatus === 'pending' && (u.role === 'buyer' || u.role === 'seller'));
    ...
    <div className="min-h-screen bg-slate-900 text-slate-50 pb-12">
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-5 gap-4 mb-8">
          ...
          <div className="bg-slate-800 p-6 rounded-2xl border border-slate-700 border-violet-500/40 ring-1 ring-violet-500/20">
            <div className="text-slate-400 text-sm font-medium mb-2 flex items-center gap-2">Pending KYC <IdCard className="w-4 h-4 text-violet-400" /></div>
            <div className="text-4xl font-bold text-violet-400">{stats.pendingKyc ?? 0}</div>
          </div>
          ...
          <div onClick={() => setActiveModal('users')} className="bg-slate-800 p-6 rounded-2xl border border-slate-700 cursor-pointer hover:bg-slate-700/50 hover:border-purple-500/50 transition-all group xl:col-span-1 md:col-span-1 col-span-2">
        ...
        <h2 className="text-2xl font-bold mb-4 flex items-center gap-2 text-violet-400"><IdCard className="w-5 h-5"/> User KYC queue ({kycPendingUsers.length})</h2>
              ...
              <tbody className="divide-y divide-slate-700/50">
                {kycPendingUsers.map(u => (
                  <tr key={u.id} className="hover:bg-slate-700/40">
                    ...
                    <td className="px-6 py-4"><span className={`px-2 py-1 rounded text-xs font-bold uppercase ${u.role === 'seller' ? 'bg-blue-500/20 text-blue-400' : 'bg-green-500/20 text-green-400'}`}>{u.role}</span></td>
                    <td className="px-6 py-4 text-slate-300 font-mono text-xs">{u.kycPhone || '—'}</td>
                    <td className="px-6 py-4 text-slate-300 text-xs">
                      <div className="text-slate-500 uppercase tracking-wide">{u.kycIdType || '—'}</div>
                      <div className="font-mono">{u.kycIdNumber || '—'}</div>
                    </td>
                    <td className="px-6 py-4 text-slate-400 text-xs max-w-xs line-clamp-3">{u.kycAddress || '—'}</td>
                    <td className="px-6 py-4 text-right">
                      <div className="flex justify-end gap-2 flex-wrap">
                        <button type="button" onClick={() => handleKycDecision(u.id, 'approved')} className="flex items-center gap-1.5 px-3 py-1.5 bg-green-500/10 text-green-400 hover:bg-green-500/20 rounded-md text-xs font-bold border border-green-500/20"><CheckCircle className="w-3.5 h-3.5"/> Approve KYC</button>
                        <button type="button" onClick={() => handleKycDecision(u.id, 'rejected')} className="flex items-center gap-1.5 px-3 py-1.5 bg-red-500/10 text-red-400 hover:bg-red-500/20 rounded-md text-xs font-bold border border-red-500/20"><XCircle className="w-3.5 h-3.5"/> Reject</button>
                      </div>
                    </td>
                  </tr>
                ))}
                {kycPendingUsers.length === 0 && (
                  <tr><td colSpan="6" className="px-6 py-12 text-center text-slate-500">No accounts waiting for KYC. New signups will appear here.</td></tr>
                )}
              </tbody>
            </table>
          </div>
                 ...
                 <div className="bg-slate-900 rounded-xl border border-slate-700 overflow-hidden">
                    <table className="w-full text-left text-sm">
                      <thead className="bg-slate-800 text-slate-400 uppercase text-xs border-b border-slate-700">
                        <tr><th className="px-6 py-4">User</th><th className="px-6 py-4">Email</th><th className="px-6 py-4">Role</th><th className="px-6 py-4">KYC</th><th className="px-6 py-4">Premium Status</th></tr>
                      </thead>
                      <tbody className="divide-y divide-slate-700/50">
                         {users.map(u => (
                           <tr key={u.id}>
                             ...
                             <td className="px-6 py-4">
                               {u.role === 'admin' ? <span className="text-slate-500 text-xs">—</span> :
                                 u.kycStatus === 'approved' ? <span className="text-green-400 text-xs font-bold">Verified</span> :
                                 u.kycStatus === 'rejected' ? <span className="text-red-400 text-xs font-bold">Rejected</span> :
                                 u.kycStatus === 'pending' ? <span className="text-violet-400 text-xs font-bold">Pending</span> :
                                 <span className="text-slate-500 text-xs">Legacy</span>}
                             </td>
